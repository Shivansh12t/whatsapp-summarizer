...
def extract_messages(chat):
    ...
    for msg in messages:
        date, time, meridian, sender, message = msg
        data.append([date, time, meridian, sender, message])
    
    if not data:
        raise ValueError("No messages extracted. Check the file format or regex pattern.")
    
    return pd.DataFrame(data, columns=["Date", "Time", "Meridian", "Sender", "Message"])
...
def process_whatsapp_chat(file_path, selected_sender=None):
    ...
    top_10_messages = df['Message_normalized'].value_counts().head(50)
    ...
    df['Hour'] = df.apply(lambda row: pd.to_datetime(f"{row['Time']} {row['Meridian']}", format="%I:%M:%S %p").hour, axis=1)
...
selected_sender = None  # Replace with 'Tanvi' or any specific name if needed
